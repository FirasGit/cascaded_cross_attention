...
class LightningClassifier(pl.LightningModule):
    ...
    def _step(self, batch, batch_idx, prefix: str):
        feats, ehr, label, label_names = batch['feats'], batch['ehr'], batch['label'], batch['label_names'][0]
        self.label_names = label_names
        
        prediction = self.get_prediction((feats, ehr))

        if self.cfg.model.get('meta', {}).get('self_supervision', {}).get('use', False):
            # Minimize negative cosine similarity
            loss = - torch.nn.functional.cosine_similarity(prediction, feats, dim=-1).mean()
            ...
            if isinstance(prediction, list):
                ...
                prediction_prob = self._get_prediction_probability(torch.stack(prediction).mean(axis=0))
            else:
