...
class TCGA_NSCLC(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = torch.tensor([0]) if sample['subtype'] == 'LUSC' else torch.tensor([1])
        label_names = ['subtype']
...
def run(cfg: DictConfig):
    train_dataset = TCGA_NSCLC(root_path = '/data/PATHO/', split='train', cfg=cfg)
